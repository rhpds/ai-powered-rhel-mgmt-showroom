...
Large Language Models (LLMs) are vulnerable to prompt injection attacks, which occur when malicious instructions are embedded into the data and LLM processes. This can trick the model into performing unauthorized or harmful actions.
...
Infinicorp will simulate a scenario where you will observe the gatekeeper model thwarting an indirect prompt injection attack.
...
.Notice that script is blocked
image::lb1305_M6_blockedscript.png[link=self, window=blank, width=100%]

NOTE: You may have to search carefully through the output.  You should see something to the screenshot above, though the exact language may vary.

After seeing the script being blocked from running you can select *Cancel* to end the prompt.
