...
= Environment Planning and Reporting

Not that you have a good handle on Infinicorp workloads and our security posture, we would appreciate you helping us to plan our rollout for the future. 
...
A tool like these MCP servers can help to query the inventory and find hosts in need of some attention. 

You should focus first on the operating system and see if any are in need of OS level upgrades.

[source, role="execute", subs="attributes"]
----
Provide a simple summary of the versions of RHEL 9 and older in use and compare to the newest versions, highlighting the number of systems that are approaching end of life. The output should be a simple to read table. Do NOT list hostnames, just a count of systems. Do NOT report on RHEL 10 versions. 
----
...
The response from the prompt may include a variety of results, possibly including a table like above that will focus on the database versions to upgrade to, other packages that need upgrading, and a migration window. +
